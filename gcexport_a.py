...
# ADRIZ-NF Adjustments to make login easier for script
...
def parse_arguments(argv):
    ...
    #ADRIZ_NF added to get number of files to skip
    parser.add_argument('-ea', '--end_activity_no', type=int, default=1,
        help="give index for activity to skip importing, i.e. skipping the oldest activities") 
...
def login_to_garmin_connect(args):
    ...
    # ADRIZ-NF edit including dotenv password
    if args.password:
        password = args.password
    else: password = PASSWORD if PASSWORD else getpass()
...
# ADRIZ_NF Added ability to limit downloaded older activities
def annotate_activity_list(activities, start, end, exclude_list):
    action_list = []
    total_activites = len(activities)
    for index, a in enumerate(activities):
        if index < (start - 1):
            action = 's'
        elif index > total_activites - end:
            action = 's'
...
def main(argv):
    ...
    action_list = annotate_activity_list(activities, args.start_activity_no, args.end_activity_no, exclude_list)
